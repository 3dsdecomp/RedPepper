...
public:
    ...
    T* popBack()
    {
        return listNodeToObjWithNullCheck(ListImpl::popBack());
    }

    T* popFront()
    {
        return listNodeToObjWithNullCheck(ListImpl::popFront());
    }
    ...
    {
        ListImpl::swap(objToListNode(obj1), objToListNode(obj2));
    }
    ...
    {
        ListImpl::mergeSort<T>(mOffset, cmp);
    }

    T* find(const T* obj) const
    {
        return find(obj, compareT);
    }
    ...
    robustIterator robustBegin(T* ptr) const
    {
        return robustIterator(ptr, mOffset);
    }
